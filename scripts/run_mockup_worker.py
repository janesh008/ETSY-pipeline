...
def run_daemon_mode(settings: object) -> None:
    """Poll MongoDB indefinitely for PENDING/FAILED mockup jobs."""
    ...
    logger.info(
        "[run_mockup_worker] Daemon mode started — polling MongoDB for mockup jobs..."
    )

    while True:
        try:
            pending_docs = store.list_jobs_by_stage_status(
                "mockups", ["PENDING", "FAILED"], limit=5
            )
